...
int rotSpeedLimit = 20; // RPM max speed of rotation
const int rotDeadZone = 10; // rotation control dead zone
bool rotZeroPosition = false; // If rotation knob is in deadzone, this is TRUE
Stepper rotStepper(stepsPerRevolution, 8, 9, 10, 11);
...
int hookSpeedLimit = 20; // RPM max speed of rotation
const int hookDeadZone = 10; // rotation control dead zone
bool hookZeroPosition = false; // If hook knob is in deadzone, this is TRUE
Stepper hookStepper(stepsPerRevolution, 4, 5, 6, 7);
...
void rotReadRoutine() {
  ...
  rotStepper.setSpeed(abs(rotMotor));
...
void hookReadRoutine() {
  ...
  hookStepper.setSpeed(abs(hookMotor));
...
void loop() {
    ...
    while (ARMED) {
      ...
      if (!rotZeroPosition) {
        ...
        rotStepper.step(rotDir * stepsPerRevolution / 50);}
      ...
      if (!hookZeroPosition) {
        ...
        rotStepper.step(hookDir * stepsPerRevolution / 50);
      }
