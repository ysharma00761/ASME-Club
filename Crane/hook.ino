...
int rotInputMin = 1023;                                          // lowest analog read
int rotInputMax = 0;                                             // highest analog read
int rotMotor = 0;                                                // The speed and direction of the rotation (positive is clockwise);
int rotSpeedLimit = 100;                                         // ticks per second max speed of rotation
const int rotDeadZone = 1;                                       // rotation control dead zone
...
int hookInputMin = 1023;        // lowest analog read
int hookInputMax = 0;           // highest analog read
...
const int hookDeadZone = 100;   // rotation control dead zone
...
void setup() {
  ...
  hookStepper.setAcceleration(200);
  rotStepper.setMaxSpeed(rotSpeedLimit);
  rotStepper.setAcceleration(200);
...
void loop() {
  ...
  while (!ARMED) {
    ...
    Serial.print("HOOK: ");
    Serial.print(hookZeroPosition);
    Serial.print(" - ");
    Serial.println(hookDir);
    Serial.print("ROTS: ");
    Serial.print(rotZeroPosition);
    Serial.print(" - ");
